...
const IconApplePodcast = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 0C5.372 0 0 5.372 0 12s5.372 12 12 12 12-5.372 12-12S18.628 0 12 0zm-.002 4.5a7.5 7.5 0 0 1 5.028 13.073 6.5 6.5 0 0 0-1.978-2.673 4.5 4.5 0 1 0-6.1 0 6.5 6.5 0 0 0-1.978 2.673A7.5 7.5 0 0 1 11.998 4.5zM12 10a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 5c1.208 0 2.28.603 2.926 1.523.322.458.574.97.733 1.518-.47.16-.96.277-1.463.35L13.5 16.5a1.5 1.5 0 0 0-3 0l-.696 1.891a7.46 7.46 0 0 1-1.463-.35c.159-.548.41-1.06.733-1.518A3.49 3.49 0 0 1 12 15z" />
  </svg>
);
  ...
  return (
    <header
      className={cn(
        ...
        'border-b',
        scrolled
          ? 'bg-[#0f0d0b]/90 dark:bg-[#0f0d0b]/90 backdrop-blur-xl border-white/5'
          : 'bg-[#0f0d0b] dark:bg-[#0f0d0b] border-transparent',
        'light:bg-white/90 light:border-zinc-200/50',
      )}
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-extrabold tracking-[0.1em] text-white dark:text-white">
            MERIDIAN
          </span>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-6">
          {['Shows', 'Episodes', 'Network', 'Community', 'About'].map(link => (
            <a
              key={link}
              href={`#${link.toLowerCase()}`}
              className="text-sm text-[#8a7e72] hover:text-white dark:text-[#8a7e72] dark:hover:text-white transition-colors duration-150"
            >
              {link}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          <button className="hidden sm:flex items-center justify-center w-8 h-8 rounded-lg text-[#8a7e72] hover:text-white transition-colors">
            <IconSearch />
          </button>
          <a
            href="#"
            className="hidden lg:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border border-[#1DB954]/50 text-[#1DB954] hover:bg-[#1DB954]/10 transition-colors"
          >
            <IconSpotify /> Follow on Spotify
          </a>
          <a
            href="#"
            className="inline-flex items-center px-4 py-2 rounded-full text-xs font-bold bg-amber-400 text-black hover:bg-amber-300 transition-colors"
          >
            Subscribe free
          </a>
        </div>
...
/* ── Show Artwork Stack ── */
function ShowArtworkStack({ shows }: { shows: Show[] }) {
  const [hovered, setHovered] = React.useState(false);
  const stackShows = shows.slice(0, 4);

  return (
    <div
      className="relative w-[280px] h-[280px] lg:w-[320px] lg:h-[320px]"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {stackShows.map((show, i) => {
        const rotations = [-6, -3, 0, 3];
        const offsets = hovered
          ? [{ x: -80, y: -20 }, { x: -30, y: -40 }, { x: 20, y: 0 }, { x: 70, y: -30 }]
          : [{ x: -8 + i * 4, y: -6 + i * 3 }, { x: -4 + i * 4, y: -3 + i * 3 }, { x: i * 4, y: i * 3 }, { x: 4 + i * 4, y: 3 + i * 3 }];
        ...
        return (
          <div
            ...
            style={{
              transform: `translate(${offsets[i].x}px, ${offsets[i].y}px) rotate(${hovered ? 0 : rotations[i]}deg)`,
              zIndex: i,
              transitionTimingFunction: 'cubic-bezier(0.34, 1.56, 0.64, 1)',
            }}
          >
            <div
              className="w-[180px] h-[180px] lg:w-[200px] lg:h-[200px] rounded-2xl overflow-hidden shadow-2xl"
              style={{
                boxShadow: `0 0 40px ${show.accentColor}33`,
              }}
            >
  ...
  return (
    <div
      className={cn(
        ...
        'bg-[#211e1a] dark:bg-[#211e1a]',
        'border border-transparent',
      )}
      style={{
        borderColor: hovered ? `${show.accentColor}50` : 'transparent',
        boxShadow: hovered ? `0 0 30px ${show.accentColor}15` : 'none',
      }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      ...
      <div className="relative aspect-square overflow-hidden">
        <img src={show.artwork} alt={show.name} className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#211e1a] via-transparent to-transparent" />
      ...
      <div className="p-4 -mt-8 relative z-10">
        <span className="text-[10px] font-semibold uppercase tracking-[0.12em] block mb-1" style={{ color: show.accentColor }}>
          {show.category}
        </span>
        <h3
          className={cn(
            'text-xl font-bold text-white italic transition-transform duration-300',
        ...
        <p className="text-xs text-[#8a7e72] mt-1 font-mono tabular-nums">{show.episodeCount} episodes</p>
        <p className="text-sm text-[#8a7e72] mt-2 line-clamp-2">{show.latestEpisode.title}</p>
      </div>
    </div>
  );
}
...
/* ── Episode Row ── */
function EpisodeRow({ episode, isPlaying, onPlay }: { episode: Episode; isPlaying: boolean; onPlay: () => void }) {
  return (
    <div className="group flex items-center gap-4 py-3 px-4 rounded-xl hover:bg-[#211e1a]/50 dark:hover:bg-[#211e1a]/50 transition-colors cursor-pointer">
      {/* Artwork thumbnail */}
      <div
        className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 ring-2"
        style={{ '--tw-ring-color': episode.accentColor + '40' } as React.CSSProperties}
      >
        <img src={episode.artwork} alt={episode.showName} className="w-full h-full object-cover" />
      </div>

      {/* Info */}
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="text-xs font-mono tabular-nums" style={{ color: episode.accentColor }}>
            #{episode.number}
          </span>
          <h4 className="text-sm font-semibold text-white dark:text-white truncate">{episode.title}</h4>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-[#8a7e72]">{episode.showName}</span>
          <span className="text-xs text-[#524840] truncate hidden sm:inline">{episode.description}</span>
        </div>
      </div>

      {/* Duration + Play */}
      <div className="flex items-center gap-3 flex-shrink-0">
        <span className="text-xs font-mono tabular-nums text-[#8a7e72]">{episode.duration}</span>
        <button
    ...
    <div className="flex items-center gap-[2px] h-6">
      {Array.from({ length: 24 }).map((_, i) => (
        <div
          key={i}
          className={cn(
            'w-[2px] rounded-full bg-amber-400/60 transition-all',
            playing && 'animate-pulse',
          )}
          style={{
            height: playing
              ? `${8 + Math.sin(i * 0.8) * 12 + Math.random() * 6}px`
  ...
  return (
    <div
      className={cn(
        ...
        'bg-[#211e1a] dark:bg-[#211e1a] border-white/[0.08]',
        episode ? 'translate-y-0' : 'translate-y-full',
      )}
    >
      <div className="mx-auto max-w-7xl h-full flex items-center justify-between px-4 gap-4">
        {/* Left: artwork + info */}
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <img src={episode.artwork} alt="" className="w-10 h-10 rounded-md flex-shrink-0" />
          <div className="min-w-0 hidden sm:block">
            <p className="text-sm font-semibold text-white truncate">{episode.title}</p>
            <p className="text-xs text-amber-400">{episode.showName}</p>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-3 flex-1 justify-end">
          ...
          <span className="text-xs font-mono tabular-nums text-[#8a7e72]">-32:15</span>
...
export interface PodcastNetworkTemplateProps {
  /** Additional className for the page wrapper */
  className?: string;
}

/**
 * PodcastNetwork — Podcast & Media Network template (Wondery / Radiotopia aesthetic).
 *
 ...
 */
export function PodcastNetworkTemplate({ className }: PodcastNetworkTemplateProps) {
  ...
  return (
    ...
    >
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6 w-full">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            {/* Left — Copy */}
            <div>
              <span className="inline-flex items-center mb-6 rounded-full px-3.5 py-1.5 text-xs font-medium bg-amber-400/10 text-amber-300 border border-amber-400/20">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.08] text-white">
                ...
                <em className="italic" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>think.</em>
              </h1>

              <p className="mt-6 text-lg leading-relaxed text-[#8a7e72] max-w-xl">
              ...
              <div className="flex flex-wrap gap-4 mt-8">
                <a href="#shows" className="inline-flex items-center px-6 py-3 rounded-xl text-sm font-bold bg-amber-400 text-black hover:bg-amber-300 transition-colors">
                  Start listening free
                </a>
                <a href="#shows" className="inline-flex items-center px-6 py-3 rounded-xl text-sm font-medium border border-white/20 text-white hover:bg-white/[0.06] transition-colors">
                  Browse all shows →
                </a>
              </div>

              <p className="mt-6 text-xs text-[#524840]">
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex justify-center">
              <div
                className="w-[300px] h-[300px] lg:w-[400px] lg:h-[400px] rounded-3xl overflow-hidden shadow-2xl"
                style={{ boxShadow: `0 0 60px ${featuredShow.accentColor}25` }}
              >
                <img src={featuredShow.artwork} alt={featuredShow.name} className="w-full h-full object-cover" />
              </div>
            </div>

            {/* Right — Details */}
            <div>
              <span className="text-[10px] font-semibold uppercase tracking-[0.15em] block mb-2" style={{ color: featuredShow.accentColor }}>
                {featuredShow.category} · Show of the Week
              </span>
              <h2
                className="text-4xl lg:text-5xl font-bold text-white italic"
                style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
              >
                {featuredShow.name}
              </h2>
              <p className="mt-3 text-[#8a7e72] text-lg">{featuredShow.tagline}</p>
              <p className="mt-4 text-sm text-[#524840] font-mono tabular-nums">{featuredShow.episodeCount} episodes</p>

              {/* Latest episode */}
              <div className="mt-6 p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-mono tabular-nums" style={{ color: featuredShow.accentColor }}>
                    #{featuredShow.latestEpisode.number}
                  </span>
                  <span className="text-xs text-[#8a7e72] font-mono tabular-nums">{featuredShow.latestEpisode.duration}</span>
                </div>
                <p className="text-sm font-semibold text-white">{featuredShow.latestEpisode.title}</p>
                <p className="text-xs text-[#524840] mt-1">{featuredShow.latestEpisode.description}</p>
              </div>
              ...
              <div className="flex items-center gap-4 mt-6">
                <button
                  ...
                  className="w-16 h-16 rounded-full flex items-center justify-center text-white hover:scale-105 transition-transform shadow-lg"
                  style={{ backgroundColor: featuredShow.accentColor }}
                >
                  <IconPlay className="w-7 h-7 ml-1" />
                </button>
                <div className="flex items-center gap-2">
                  <a href="#" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs border border-white/10 text-[#8a7e72] hover:text-white hover:border-white/20 transition-colors">
                    <IconSpotify /> Spotify
                  </a>
                  <a href="#" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs border border-white/10 text-[#8a7e72] hover:text-white hover:border-white/20 transition-colors">
                    <IconApplePodcast /> Apple
                  </a>
                </div>
              </div>
            </div>
      ...
      {/* ── Section 2 — All Shows ── */}
      <section id="shows" className="py-16 lg:py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
            <h2 className="text-3xl lg:text-4xl font-extrabold tracking-tight text-white">THE NETWORK</h2>
            {/* Filter pills */}
      ...
      <section className="py-16 lg:py-24 border-t border-white/5">
        <div className="mx-auto max-w-7xl px-6">
          <h2 className="text-2xl lg:text-3xl font-extrabold tracking-tight text-white mb-8">JUST RELEASED</h2>
      ...
      <section className="py-16 lg:py-24 bg-[#0a0806] dark:bg-[#0a0806] relative">
        {/* Subtle noise */}
        <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=\'0 0 256 256\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cfilter id=\'noise\'%3E%3CfeTurbulence type=\'fractalNoise\' baseFrequency=\'0.65\' numOctaves=\'3\' stitchTiles=\'stitch\'/%3E%3C/filter%3E%3Crect width=\'100%25\' height=\'100%25\' filter=\'url(%23noise)\'/%3E%3C/svg%3E")', backgroundRepeat: 'repeat' }} />

        <div className="relative z-10 mx-auto max-w-2xl px-6 text-center">
          <h2 className="text-3xl lg:text-4xl font-extrabold text-white">Join the Meridian community.</h2>
          <p className="mt-4 text-[#8a7e72] text-lg">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl bg-[#211e1a] border border-white/10 text-white placeholder-[#524840] text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50 transition-shadow"
            />
            <button className="px-6 py-3 rounded-xl text-sm font-bold bg-amber-400 text-black hover:bg-amber-300 transition-colors whitespace-nowrap">
              Subscribe
            </button>
          </div>

          <p className="mt-4 text-xs text-[#524840]">
            No spam. Unsubscribe anytime. Already 84,000 subscribers.
          </p>

          {/* Member benefit pills */}
          <div className="flex flex-wrap justify-center gap-2 mt-6">
            {['Early access', 'Ad-free feed', 'Bonus episodes'].map(benefit => (
              <span key={benefit} className="px-3 py-1 rounded-full text-xs font-medium bg-amber-400/10 text-amber-300 border border-amber-400/20">
                {benefit}
              </span>
            ))}
          </div>
      ...
      <section className="py-16 lg:py-24 border-t border-white/5">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            {/* Left — Team photo */}
            <div className="rounded-2xl overflow-hidden bg-[#211e1a] aspect-[4/3]">
              <img
            ...
            {/* Right — Manifesto */}
            <div>
              <h2 className="text-3xl lg:text-4xl font-extrabold text-white mb-6">The Network</h2>
              <p className="text-[#8a7e72] text-lg leading-relaxed mb-4">
              ...
              <p className="text-[#8a7e72] text-lg leading-relaxed mb-8">
              ...
              <div className="flex flex-wrap gap-6 text-center">
                {[
                  { num: '8', label: 'shows' },
                  { num: '2M', label: 'listeners' },
                  { num: '143', label: 'episodes' },
                  { num: '4', label: 'years independent' },
                ].map(st => (
                  <div key={st.label}>
                    <div className="text-2xl font-extrabold text-amber-400 font-mono tabular-nums">{st.num}</div>
                    <div className="text-xs text-[#524840] mt-1">{st.label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Host grid */}
          <div className="mt-16">
            <h3 className="text-sm font-semibold uppercase tracking-[0.12em] text-[#524840] mb-6">Our Hosts</h3>
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-4">
              {HOSTS.map(host => (
                <div key={host.name} className="text-center">
                  <div className="w-16 h-16 mx-auto rounded-full overflow-hidden bg-[#211e1a] mb-2">
                  ...
                  <p className="text-[10px] text-[#524840]">{host.show}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <footer className="border-t border-white/5 py-12">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid sm:grid-cols-4 gap-8">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                <span className="text-amber-400 text-lg">◉</span>
                <span className="text-sm font-extrabold tracking-[0.1em] text-white">MERIDIAN</span>
              </div>
              <p className="text-xs text-[#524840] leading-relaxed">
            ...
            {[
              ...
              <div key={group.title}>
                <h4 className="text-xs font-semibold uppercase tracking-[0.12em] text-[#8a7e72] mb-3">{group.title}</h4>
                <ul className="space-y-2">
                  {group.links.map(link => (
                    <li key={link}>
                      <a href="#" className="text-sm text-[#524840] hover:text-white transition-colors">{link}</a>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="mt-12 pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-[#524840]">© {new Date().getFullYear()} Meridian Audio. All rights reserved.</p>
            <div className="flex items-center gap-4">
              <a href="#" className="text-xs text-[#524840] hover:text-white transition-colors">Privacy</a>
              <a href="#" className="text-xs text-[#524840] hover:text-white transition-colors">Terms</a>
              <a href="#" className="text-xs text-[#524840] hover:text-white transition-colors">Accessibility</a>
            </div>
          </div>
        </div>
      </footer>
      ...
      <MiniPlayer episode={currentEpisode} isPlaying={isPlaying} onToggle={() => setIsPlaying(!isPlaying)} />

      {/* Bottom spacer for mini player */}
      {currentEpisode && <div className="h-14" />}
    </div>
  );
}

PodcastNetworkTemplate.displayName = 'PodcastNetworkTemplate';
